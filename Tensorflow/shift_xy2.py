...
x_v = tf.convert_to_tensor(shift,dtype=tf.float32)
#mul_x = float(x*x_v)
#mul_y = x*y_val
#print(mul_x)
...
offset_height = 0
offset_width = 0
target_height = image.shape[0] - 0
target_width = image.shape[1] - t 
#aug = tfa.image.translate_xy(image,t,0)
aug = tf.image.crop_to_bounding_box(image, offset_height, offset_width, target_height, target_width)
aug = tf.image.pad_to_bounding_box(aug,0,t,image.shape[0],image.shape[1])
